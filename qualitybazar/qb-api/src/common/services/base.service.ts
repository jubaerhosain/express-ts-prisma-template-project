...
import { DeletionType, FindAllOptions, FindOneOptions, MapperOptions } from "@common/types";
...
> implements IBaseService<TModel, TCreateDto, TUpdateDto, TResponseDto>
{
  ...
  async findOne(options: FindOneOptions<TModel>): Promise<TResponseDto> {
  ...
  async findAll(query?: BaseQueryDto): Promise<TResponseDto[]> {
    const list = await this._repository.findAll(query);
    const mappedList = this._mapper.mapArray<TModel, TResponseDto>(list, this._model, this._responseDto);
    return mappedList;
  }

  async findAllPaginated(query?: BaseQueryDto): Promise<BasePaginatedResponseDto> {
    return this.paginate(query);
  }

  async paginate(options: FindAllOptions<TModel>): Promise<BasePaginatedResponseDto> {

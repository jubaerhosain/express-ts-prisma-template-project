  ...
  implements IOrderService
{
  ...
  override async findAllPaginated(queryDto: OrderQueryDto): Promise<PaginatedOrderDto> {
    const orders = await this.orderRepository.findAll(queryDto);

    const { page, limit } = queryDto;
    const itemCount = orders.length;

    return {
      items: this.mapper.mapArray(orders, Order, OrderDto),
      meta: await this.getPaginationMeta({ itemCount, page, limit }),
    };
  }
}
